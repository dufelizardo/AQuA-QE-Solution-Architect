...
- **Piloto de provedor em nuvem via toggle** (`LLM_PROVIDER=ollama|nvidia|cerebras|google`) — mesmo piloto já validado ao vivo no agente irmão AQuA-QE Product Manager, replicado e expandido aqui. NVIDIA NIM (`build.nvidia.com`) foi o primeiro testado, mas se mostrou instável ao vivo (capacidade, entitlement, timeout); Cerebras Inference (`api.cerebras.ai`) respondeu rápido mas exigiu billing pendente na conta (402); **Google AI Studio** (`generativelanguage.googleapis.com`, `gemini-3.1-flash-lite`/`gemini-2.5-flash-lite`) foi o único que completou um Solution Design real de ponta a ponta com sucesso. Todos preservam o princípio de dois modelos independentes (gerador/revisor). Ollama continua o padrão inalterado quando `LLM_PROVIDER` não é definido.
...
- **Frameworks formais de descrição/avaliação de arquitetura** (TOGAF, Zachman, Arc42, ISO/IEC/IEEE 42010, ISO/IEC 12207, QAW, ATAM) — citados na especificação original do agente para fundamentar como documentar e avaliar formalmente uma arquitetura; nenhum foi adotado nesta fase (`knowledge/methodology/` cobre só o necessário ao escopo atual). Adotá-los sem um consumidor real (ex.: um cliente exigindo documentação formal em Arc42) seria especulativo.
