...
- **Piloto de provedor alternativo via toggle** (`LLM_PROVIDER=ollama|nvidia`, padrão `ollama`): mesmo piloto já validado ao vivo no agente irmão AQuA-QE Product Manager, replicado aqui — NVIDIA NIM (`build.nvidia.com`, API compatível com OpenAI) como provedor opcional de gerador/revisor, mantendo Ollama como padrão inalterado. Modelos confirmados funcionando na mesma conta: `deepseek-ai/deepseek-v4-pro` (gerador) e `meta/llama-3.3-70b-instruct` (revisor). Decisão consciente de reverter, apenas para o piloto, o princípio "modelos locais via Ollama" descrito no `WHITEPAPER.md` — não uma substituição definitiva.
